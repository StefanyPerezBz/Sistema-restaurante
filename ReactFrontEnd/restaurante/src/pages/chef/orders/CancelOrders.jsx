...
export default function CancelOrders() {
    ...
    return (
        <div className='w-full min-h-screen bg-gray-100 p-4'>
                    ...
                    <Navbar.Collapse className="flex flex-wrap justify-center md:justify-start">
                        ...
                        <Link to="/chef?tab=canceledOrders" className="m-1">
                            <Button color="failure" pill active>
                                Cancelados: {filteredOrders.length}
                            </Button>
                        </Link>
                        <div className="m-1">
                            <Button 
            ...
            <Label className='text-2xl font-bold mb-4'>Pedidos cancelados - {new Date().toLocaleDateString()}</Label>
            
            {viewMode === 'table' ? (
                <div className="bg-white shadow-md rounded-2xl p-4 overflow-x-auto">
                    {filteredOrders.length === 0 ? (
                        <div className="text-center py-8">
                            <p className="text-gray-500 text-lg">No hay pedidos cancelados</p>
                        </div>
                    ) : (
                        <DataTable
                            columns={columns}
                            ...
                            responsive
                            highlightOnHover
                            noDataComponent={<div className="py-4">No hay pedidos cancelados</div>}
                        />
                    )}
                </div>
            ) : (
