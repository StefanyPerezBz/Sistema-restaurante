...
export default function ManageOrder() {
    ...
    const handleCancelOrder = async (order) => {
        ...
        try {
            const response = await fetch(`localhost:8080/api/orders/status-update/${orderId}/Canceled`, {
                method: 'PUT'
            });
            if (response.status === 204 || response.ok) {
                ...
                if (order.tableNumber > 0) {
                    ...
                    const tableResponse = await axios.put(`localhost:8080/api/table/${order.tableNumber}/availability?availability=true`);
