...
const ViewAllEvents = () => {
  ...
  useEffect(() => {
    const viewEvents = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`localhost:8080/api/events/view-events`);
  ...
  const handleDelete = async (eventID) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(`localhost:8080/api/events/delete/${eventID}`);
  ...
  const fetchEventDetails = async (eventID) => {
    try {
      const response = await axios.get(`localhost:8080/api/inform/get/${eventID}`);
  ...
  const handleShare = async (eventID) => {
    try {
      ...
      if (emailCount) {
        ...
        const response = await axios.post(`localhost:8080/api/inform/share-event-details`, {
          eventID: eventID,
